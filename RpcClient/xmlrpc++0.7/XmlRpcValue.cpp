...
namespace XmlRpc {
  ...
  {
    char buf[256];
    _snprintf_s(buf, sizeof(buf)-1, "%d", _value.asInt);
    buf[sizeof(buf)-1] = 0;
    std::string xml = VALUE_TAG;
    xml += I4_TAG;
    xml += buf;
  ...
  {
    char buf[256];
    _snprintf_s(buf, sizeof(buf)-1, getDoubleFormat().c_str(), _value.asDouble);
    buf[sizeof(buf)-1] = 0;
  ...
  std::string XmlRpcValue::timeToXml() const
  {
    ...
    char buf[20];
    _snprintf_s(buf, sizeof(buf)-1, "%4d%02d%02dT%02d:%02d:%02d", 
