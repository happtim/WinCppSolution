...
public:
  ...
  void async_write(const T& t, Handler handler)
  {
	...
	std::ostringstream header_stream;
	header_stream << std::setw(header_length) << std::hex << outbound_data_.length();
  ...
  void async_read(T& t, Handler handler)
  {
    ...
    asio::async_read(socket_, asio::buffer(inbound_header_),
        std::bind(f, this, _1, std::ref(t), std::make_tuple(handler)));
  }
  ...
  void handle_read_header(const asio::error_code& e,  T& t, std::tuple<Handler> handler)
  {
    ...
    else
    {
      ...
      asio::async_read(socket_, asio::buffer(inbound_data_),
        std::bind(f, this, _1, std::ref(t), handler));
    }
  }
