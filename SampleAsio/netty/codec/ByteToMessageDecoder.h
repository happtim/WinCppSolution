...
class ByteToMessageDecoder : public InboundHandler< std::vector< asio::mutable_buffer>&, M> {
 public:
  //typedef typename InboundHandler<folly::IOBufQueue&, M>::Context Context;
	typedef typename InboundHandler<std::vector< asio::mutable_buffer>&, M>::Context Context;
  ...
  //virtual bool decode(Context* ctx, folly::IOBufQueue& buf, M& result, size_t&) = 0;
	virtual bool decode(Context* ctx, std::vector< asio::mutable_buffer>& buf, M& result, size_t&) = 0;
  ...
  void read(Context* ctx, std::vector<asio::mutable_buffer>& q) override {
    bool success = true;
...
typedef ByteToMessageDecoder<std::unique_ptr<asio::mutable_buffer>> ByteToByteDecoder;
