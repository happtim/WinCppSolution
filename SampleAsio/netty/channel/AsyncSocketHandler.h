...
class AsyncSocketHandler  : public BytesToBytesHandler{
 ...
 // std::future<folly::Unit> write( Context* ctx,std::unique_ptr<folly::IOBuf> buf) override {
  std::future<folly::Unit> write(Context* ctx, std::unique_ptr<asio::mutable_buffer> buf) override {
    refreshTimeout();
    if (UNLIKELY(!buf)) {
	...
	std::future<unsigned int>  future = socket_->async_write_some(*buf, asio::use_future);
  ...
  std::future<folly::Unit> writeException(Context* ctx,
                                            folly::exception_wrapper) override {
    return shutdown(ctx, true);
  }

  std::future<folly::Unit> close(Context* ctx) override {
 ...
 private:
  ...
  std::future<folly::Unit> shutdown(Context* ctx, bool closeWithReset) {
    if (socket_) {
      detachReadCallback();
      if (closeWithReset) {
