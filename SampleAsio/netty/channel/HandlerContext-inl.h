...
class InboundLink {
 public:
  ...
  virtual void readException(std::exception e) = 0;
...
class OutboundLink {
 public:
  ...
  virtual std::future<folly::Unit> write(Out msg) = 0;
  virtual std::future<folly::Unit> writeException( std::exception e) = 0;
  virtual std::future<folly::Unit> close() = 0;
};
 ...
 public:
  ...
  void setNextIn(PipelineContext* ctx) override {
    ...
    if (nextIn) {
      ...
      throw std::invalid_argument( fmt::format("inbound type mismatch after {}", typeid(H).name() ));
    }
  }
  ...
  void setNextOut(PipelineContext* ctx) override {
    ...
    if (nextOut) {
      ...
      throw std::invalid_argument( fmt::format( "outbound type mismatch after {}", typeid(H).name() ));
    }
  }
...
class ContextImpl
 ...
 public:
  ...
  void fireReadException(std::exception e) override {
    auto guard = this->pipelineWeak_.lock();
    if (this->nextIn_) {
  ...
  std::future<folly::Unit> fireWrite(Wout msg) override {
    auto guard = this->pipelineWeak_.lock();
    if (this->nextOut_) {
	  ...
	  std::promise<folly::Unit> promise;
	  return promise.get_future();

	  //return std::makeFuture();
    }
  }

  std::future<folly::Unit> fireWriteException( std::exception e) override {
    auto guard = this->pipelineWeak_.lock();
    if (this->nextOut_) {
  ...
  std::future<folly::Unit> fireClose() override {
    auto guard = this->pipelineWeak_.lock();
    if (this->nextOut_) {
  ...
  void readException(std::exception e) override {
    auto guard = this->pipelineWeak_.lock();
  ...
  std::future<folly::Unit> write(Win msg) override {
    auto guard = this->pipelineWeak_.lock();
    return this->handler_->write(this, std::forward<Win>(msg));
  }

  std::future<folly::Unit> writeException(  std::exception e) override {
    auto guard = this->pipelineWeak_.lock();
    return this->handler_->writeException(this, std::move(e));
  }

  std::future<folly::Unit> close() override {
    auto guard = this->pipelineWeak_.lock();
...
class OutboundContextImpl
 ...
 public:
  ...
  explicit OutboundContextImpl( std::weak_ptr<PipelineBase> pipeline, std::shared_ptr<H> handler) {
    this->impl_ = this;
    this->initialize(pipeline, std::move(handler));
  }
