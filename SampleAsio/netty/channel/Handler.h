...
namespace netty {
	...
	public:
		...
		virtual void readException(Context* ctx, std::exception e) {
		...
		virtual std::future<folly::Unit> write(Context* ctx, Win msg) = 0;
		virtual std::future<folly::Unit> writeException(Context* ctx, std::exception e) {
			return ctx->fireWriteException(std::move(e));
		}
		virtual std::future<folly::Unit> close(Context* ctx) {
	...
	public:
		...
		virtual std::future<folly::Unit> write(Context* ctx, Win msg) = 0;
		virtual std::future<folly::Unit> writeException(
			Context* ctx, std::exception e) {
			return ctx->fireWriteException(std::move(e));
		}
		virtual std::future<folly::Unit> close(Context* ctx) {
			return ctx->fireClose();
	...
	public:
		...
		std::future<folly::Unit> write(Context* ctx, W msg) override {
	...
	typedef HandlerAdapter<std::vector<asio::mutable_buffer>&, std::unique_ptr<asio::mutable_buffer>>
		BytesToBytesHandler;

	//typedef InboundHandler<folly::IOBufQueue&, std::unique_ptr<folly::IOBuf>>
	//	InboundBytesToBytesHandler;

	typedef InboundHandler<std::vector<asio::mutable_buffer>&, std::unique_ptr<asio::mutable_buffer>>
		InboundBytesToBytesHandler;

	//typedef OutboundHandler<std::unique_ptr<folly::IOBuf>>
	//	OutboundBytesToBytesHandler;

	typedef OutboundHandler<std::unique_ptr<asio::mutable_buffer>>
		OutboundBytesToBytesHandler;
